...
tags: [hackthebox, windows-ad, shadow-credentials, wsus-spoofing, adcs, esc17, dns-hijacking, dll-hijacking, laps]
tag_anchors:
  shadow-credentials: "#3-lateral-movement-shadow-credentials-abuse"
  wsus-spoofing: "#6-privilege-escalation-wsus-spoofing"
  adcs: "#64-tls-certificate-generation-ad-cs"
  esc17: "#62-wsus-misconfiguration-discovery"
  dns-hijacking: "#63-dns-hijacking-via-impacket-dnstoolpy"
  dll-hijacking: "#5-updatemonitor-dll-hijacking"
---

### TL;DR

Like most real-world engagements, we start with a low-privilege account — `wallace.everette`:`Welcome2026@` — handed over as part of the assessment scope. From there, I found a leaked service password in the `Logs` SMB share — `svc_recovery`'s creds, but a year out of date. Mutated the year (2025 → 2026), dodged the DC's clock skew with `fake_time`, and got a working Kerberos login. BloodHound showed `svc_recovery` had GenericWrite over the `MSA_HEALTH$` machine account, so I ran a Shadow Credentials attack with Certipy to mint a TGT and NT hash for it — then pass-the-hash'd into WinRM as `msa_health$`. On the DC I found a scheduled task running `UpdateMonitor.exe` out of a directory the IT group could write to, so I DLL-hijacked `settings_update.dll` and caught a shell as `jaylee.clifton` (user flag). From there, a support ticket spilled the whole plan: the box pulls WSUS updates over HTTPS from `wsus.logging.htb` every two minutes. So I hijacked the DNS record, abused AD CS (ESC17) to get a domain-trusted cert for that hostname, stood up a rogue WSUS server, and served a fake "update" that dropped me SYSTEM (root flag from `toby.brynleigh`). Reset the Administrator password, dumped NTDS.dit, full domain compromise. 🔥
...
## 2.3 Password Mutation & Credential Validation
...
### 2.3.1 Manual Mutation & Clock Skew Evasion
...
## 2.4 Kerberos Ticket Generation
...
## 2.5 Targeted BloodHound Enumeration
...
### 2.5.1 Privilege Analysis: svc_recovery
...
<p align="center">
  <img src="/assets/images/htb-logging-bh_svc_recovery_genericwrite.png" alt="BloodHound - svc_recovery GenericWrite">
</p>
...
> Note: `bloodyAD` can also execute Shadow Credentials attacks, but reliability depends on DC configuration and Kerberos strictness. See [Appendix A](#appendix-a-technical-deep-dive--pkinit-failures) for technical details on PKINIT failures.
...
<p align="center">
  <img src="/assets/images/htb-logging-bh_admin_path.png" alt="BloodHound - Domain Admin Nesting">
</p>
...
![dnSpy - UpdateMonitor Path Construction](/assets/images/htb-logging-dnspy_paths.png)
...
![dnSpy - LoadLibrary P/Invoke](/assets/images/htb-logging-dnspy_loadlibrary.png)
...
## 5.5 Alternative Payload Generation & Lateral Movement (Metasploit)
...
<p align="center">
  <img src="/assets/images/htb-logging-wsus_ticket.png" alt="Support Incident View">
</p>
...
### A.1 Two Outcomes, One Command — bloodyAD & PKINIT

`bloodyAD` is incredibly efficient at injecting Shadow Credentials via LDAP — but its built-in **PKINIT** step is flaky in strict Kerberos environments. Same command, two very different outcomes across my runs.

**Run 1 — success.** The key credential is written and the TGT + NT hash fall straight out:
...
**Run 2 — failure.** Identical command on the same box; this time the key pair is generated and written to `msDS-KeyCredentialLink`, but the PKINIT pre-authentication phase crashes before the TGT comes back:
...
This is why my rule of thumb became: **bloodyAD for LDAP operations, Certipy for anything Kerberos/PKINIT** — and when a bloodyAD Shadow Credentials run does fail mid-PKINIT, all is not lost (next section).

### A.2 Remediation — Completing Auth with Certipy
...
### C.1 Extracting the TGT with Rubeus
...
### C.2 Converting to Linux Format (.ccache)
...
### C.4 Remote DNS Hijacking via Kerberos
...
### C.5 Remote Enrollment via Certipy
...
### C.6 Final Certificate Preparation
...
### C.7 Executing the Attack & Final Compromise
